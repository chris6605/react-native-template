class DateUtils {
   ...
   *
   */
  static diff(startDateStr, endDateStr, unitOfTime = 'days') {
    if (!startDateStr || !endDateStr) {
      return ''
    }
    const startTime = Date.parse(new Date(startDateStr))
    const endTime = Date.parse(new Date(endDateStr))
  ...
  static formateChatTime(dateStr) {
    if (!dateStr) {
      return
    }
    ...
    const delay = Math.round((Date.parse(now) - Date.parse(target)) / 1000 / 3600 / 24) // 时间差
    // 时间在一天之内只返回时分
    if (delay === 0) {
