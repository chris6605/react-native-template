...
export default class InformationPager extends BaseComponent {
  page = 1
  page_size = 20
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      data: [1, 2, 3, 4, 5, 6],
      refreshing: false,
      loadStatus: 0, //0 无  1上拉加载更多  2没有更数据了  3正在加载
    }
  }
  ...
  renderItem(item) {
    return (
      <TouchableOpacity
        ...
        onPress={() => {}}>
        <Text style={{ fontSize: Const.getSize(16), color: Const.blackColor }}>item</Text>
      </TouchableOpacity>
    )
  }
    ...
    if (this.state.loadStatus === 0) {
      ...
      footer = (
        <View
          ...
          <ActivityIndicator animating={true} color={Const.grayColor} size="small" />
        </View>
      )
    }
    return footer
